namespace PopStudio.Atlas
{
    internal class CutImage
    {
        public static Dictionary<string, SubImageInfo> Splice(string inFolder, string outFile, int width, int height)
        {
            using (IDisposablePool pool = new IDisposablePool())
            {
                ...
                foreach (string s in all)
                {
                    ...
                    {
                        YFLib.Bitmap img = pool.Add(YFLib.Bitmap.Create(s));
                ...
                using (YFLib.Bitmap bitmap = YFLib.Bitmap.Create(width, height))
                {
                    for (int i = 0; i < c; i++)
                    {
        ...
        public static void Cut(string inFile, string outFolder, List<SubImageInfo> cutinfo)
        {
            ...
            using (YFLib.Bitmap bitmap = YFLib.Bitmap.Create(inFile))
            {
                foreach (SubImageInfo info in cutinfo)
                {
                    ...
                    if (info.rotate270)
                    {
                        using (YFLib.Bitmap map = bitmap.Cut(info.X, info.Y, info.Height, info.Width))
                        {
                            using (YFLib.Bitmap mp2 = map.Rotate270())
                            {
                                mp2.Save(p);
                            }
                        }
                    ...
                    {
                        using (YFLib.Bitmap map = bitmap.Cut(info.X, info.Y, info.Width, info.Height))
                        {
                            map.Save(p);
                        }
                    }
