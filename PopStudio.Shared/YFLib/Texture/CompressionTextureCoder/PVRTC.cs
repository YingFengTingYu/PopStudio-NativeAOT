...
{
    internal unsafe class PVRTC
    {
        public static void DecodePVRTCI_4BPP(byte* color_byteptr, int width, byte* packets_byteptr)
        {
            int blocks = width >> 2;
            int blockMask = blocks - 1;
            uint* result = (uint*)color_byteptr;
            PvrTcPacket* packets = (PvrTcPacket*)packets_byteptr;
                ...
                for (int x = 0; x < blocks; x++)
                {
                    ...
                    for (int py = 0; py < 4; py++)
                    {
                        ...
                        for (int px = 0; px < 4; px++)
                        {
                            ...
                            result[(py + (y << 2)) * width + px + (x << 2)] = (uint)((ca.r * weights[index] + cb.r * weights[index + 1]) >> 7 << 16) | (uint)((ca.g * weights[index] + cb.g * weights[index + 1]) >> 7 << 8) | (uint)((ca.b * weights[index] + cb.b * weights[index + 1]) >> 7) | (uint)((ca.a * weights[index + 2] + cb.a * weights[index + 3]) >> 7 << 24);
                            mod >>= 2;
                            factorindex++;
                        }
                    }
        ...
        public static void EncodePVRTCI_4BPP_RGBA(byte* colors_byteptr, int width, byte* packets_byteptr)
        {
            int blocks = width >> 2;
            int blockMask = blocks - 1;
            BGRAColor* colors = (BGRAColor*)colors_byteptr;
            PvrTcPacket* packets = (PvrTcPacket*)packets_byteptr;
            for (int y = 0; y < blocks; y++)
            {
                for (int x = 0; x < blocks; x++)
                {
                    CalculateBoundingBox(colors, width, x, y, out BGRAColor min, out BGRAColor max);
                ...
                for (int x = 0; x < blocks; x++)
                {
                    ...
                    for (int py = 0; py < 4; py++)
                    {
                        ...
                        for (int px = 0; px < 4; px++)
                        {
                            ...
                            ColorRGBA ca = p0.GetColorA_ColorRGBA() * factor[0] + p1.GetColorA_ColorRGBA() * factor[1] + p2.GetColorA_ColorRGBA() * factor[2] + p3.GetColorA_ColorRGBA() * factor[3];
                            ColorRGBA cb = p0.GetColorB_ColorRGBA() * factor[0] + p1.GetColorB_ColorRGBA() * factor[1] + p2.GetColorB_ColorRGBA() * factor[2] + p3.GetColorB_ColorRGBA() * factor[3];
                            BGRAColor pixel = colors[dataindex + py * width + px];
                            ColorRGBA d = cb - ca;
        ...
        public static void EncodePVRTCI_4BPP_RGB(byte* colors_byteptr, int width, byte* packets_byteptr)
        {
            int blocks = width >> 2;
            int blockMask = blocks - 1;
            BGRAColor* colors = (BGRAColor*)colors_byteptr;
        ...
        static void CalculateBoundingBox(BGRAColor* colors, int width, int blockX, int blockY, out BGRAColor min, out BGRAColor max)
        {
            //same as DXT
            ...
            min = new BGRAColor(minr, ming, minb, mina);
            max = new BGRAColor(maxr, maxg, maxb, maxa);
        }
        ...
        private struct PvrTcPacket
        {
            ...
            public BGRAColor GetColorA()
            {
                ...
                if (colorAIsOpaque)
                {
                    ...
                    int g = (colorA >> 4) & 0x1F;
                    int b = colorA & 0xF;
                    return new BGRAColor((byte)((r << 3) | (r >> 2)), (byte)((g << 3) | (g >> 2)), (byte)((b << 4) | b));
                }
                else
                {
                    int a = (colorA >> 11) & 0x7;
                    ...
                    int g = (colorA >> 3) & 0xF;
                    int b = colorA & 0x7;
                    return new BGRAColor((byte)((r << 4) | r), (byte)((g << 4) | g), (byte)((b << 5) | (b << 2) | (b >> 1)), (byte)((a << 5) | (a << 2) | (a >> 1)));
                }
            }

            public BGRAColor GetColorB()
            {
                ...
                if (colorBIsOpaque)
                {
                    ...
                    int g = (colorB >> 5) & 0x1F;
                    int b = colorB & 0x1F;
                    return new BGRAColor((byte)((r << 3) | (r >> 2)), (byte)((g << 3) | (g >> 2)), (byte)((b << 3) | (b >> 2)));
                }
                else
                {
                    int a = (colorB >> 12) & 0x7;
                    ...
                    int g = (colorB >> 4) & 0xF;
                    int b = colorB & 0xF;
                    return new BGRAColor((byte)((r << 4) | r), (byte)((g << 4) | g), (byte)((b << 4) | b), (byte)((a << 5) | (a << 2) | (a >> 1)));
                }
            }
            ...
            public void SetColorA_RGBA(BGRAColor color)
            {
                int a = color.Alpha >> 5;
                if (a == 0x7)
                {
            ...
            public void SetColorB_RGBA(BGRAColor color)
            {
                int a = color.Alpha >> 5;
                if (a == 0x7)
                {
            ...
            public void SetColorA_RGB(BGRAColor color)
            {
                int r = color.Red >> 3;
                int g = color.Green >> 3;
                int b = color.Blue >> 4;
            ...
            public void SetColorB_RGB(BGRAColor color)
            {
                int r = color.Red >> 3;
                int g = color.Green >> 3;
                int b = color.Blue >> 3;
