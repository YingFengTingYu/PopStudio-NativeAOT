                ...
                fixed (byte* fixedptr = info.Data)
                {
                    ...
                    for (int i = 0; i < height; i += 4)
                    {
                        for (int w = 0; w < width; w += 4)
                        {
                            ...
                            CompressionTextureCoder.DXTC.DecodeLinearAlphaWord_SmallEndian(texptr, color_ptr);
                            texptr += 16;
                        }
                    }
                }
                ...
                fixed (byte* fixedptr = ans)
                {
                    ...
                    for (int i = 0; i < height; i += 4)
                    {
                        for (int w = 0; w < width; w += 4)
                        {
                            ...
                            CompressionTextureCoder.DXTC.EncodeLinearColorWord_SmallEndian_RGB_FastestSpeed(texptr + 8, color_ptr);
                            CompressionTextureCoder.DXTC.EncodeLinearAlphaWord_SmallEndian(texptr, color_ptr);
                            texptr += 16;
                        }
                    }
                }
